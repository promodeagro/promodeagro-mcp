...
# This script helps you set up the complete GitHub Actions integration for the UI project
...
if [ -n "$OIDC_PROVIDER_ARN" ]; then
    ...
    aws cloudformation deploy \
        --template-file ./git-workflow/github-oidc-setup.yml \
        --stack-name github-actions-oidc-alert-engine-ui \
        --parameter-overrides \
            GitHubOrganization=$GITHUB_ORG \
            GitHubRepository=$GITHUB_REPO \
            BranchName=$BRANCH_NAME \
    ...
    if [ $? -eq 0 ]; then
        ...
        print_color $BLUE "./git-workflow/setup-github-integration-simple.sh"
        exit 1
    fi
...
echo "• Blue/green deployment with CloudFront origins"
